...
def editWork(request, id):
    ...
    if request.method == "GET":
        if work.imageLink == """style="width:700px;""":
            work.imageLink = ""
            work.imageOnHoverLink = ""
            work.save()
        return render(request, "por/editWork.html", {
            "work":work
        })
    elif request.method == "POST":
        title = request.POST["title"]
        ...
        work.imageLink = imageLink
        work.imageOnHoverLink = imageOnHoverLink
        work.save()
        return HttpResponseRedirect(reverse("yourPor"))